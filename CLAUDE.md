...
- `victron/report.py` — `build_figure()` (shared with Dash app) + `generate_html()` + CLI; interactive Plotly HTML report with 5 subplots:
  ...
  - `--days N` and `--no-open` flags; reports saved in `reports/`
