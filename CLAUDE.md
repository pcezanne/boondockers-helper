...
├── reports/                  ← generated HTML reports (wiped often — do not store studies here)
├── studies/                  ← persistent analysis write-ups and case studies
├── analyze_door_effect.py    ← ad-hoc script: compares shore charging with bay door open vs closed
...
- `boondockers/report.py` — `build_figure()` (shared with Dash app) + `generate_html()` + CLI; interactive Plotly HTML report with 5 subplots:
  ...
  - SOC over time with adaptive LTTB downsampling (3 age tiers: full res <6h, 5-min <24h, 15-min older) to keep HTML compact; session boundaries always pinned at full resolution; sessions with notes show the note as a hover tooltip over the shaded region (invisible filled rect trace; SOC line hover takes priority when on the line); default viewport = last 3 days (`window_days=3` param in `build_figure()`); rangeselector buttons (3d / 7d / 14d / 30d / All) above the SOC chart for quick navigation — note: `xaxis_rangeselector` targets only row 1; `update_xaxes(rangeselector=...)` would apply to all rows and break layout; all subplots scroll together (shared x-axis); ticks always 1-day spacing (Plotly auto-skips overlap when zoomed out); active button pre-selected based on `window_days` CLI flag (gray inactive, dark navy active, white text on both); `active` index injected via `fig.layout['xaxis']['rangeselector']._props['active']` — both `update_layout()` and `fig.layout[...][key] =` go through Plotly's Python validator which rejects `active`; writing to `_props` directly bypasses validation while still landing in the serialized JSON that Plotly.js renders
  ...
  - **CC/CV phase analysis**: Lithium (LiFePO4) charging is two phases — CC (constant current, voltage rising) then CV (constant voltage, current tapering). `_session_started_in_cv(session_readings)` detects sessions where the charger was already in CV when logging began (battery voltage was at the CV setpoint before the session started — no CC phase was captured). Detection uses two conditions: (1) first readings are >15% above the middle-50% plateau estimate, AND (2) the middle section of the session is still declining >20% (distinguishes from thermal derating, which has a stable post-derating CC plateau). When `started_in_cv=True`: `cc_rate_pct_per_hour=None`, `knee_soc=None`, `cv_detected=False`; session is excluded from charge-rate trending and charge rate chart bars. `_find_cc_cv_knee(session_readings)` is only called when `started_in_cv=False`; it detects the CC→CV transition by finding where smoothed current drops below 85% of the plateau; returns the index of the knee reading, or `None` if no CV phase is detected (session too short, session ended in CC phase, or current never tapers); `charging_session_stats()` returns: `cc_rate_pct_per_hour` (rate for CC phase only; `None` for CV-start sessions, equals full-session rate when no CV detected), `knee_soc` (SOC at the knee reading; `None` when no CV detected or CV-start), `cv_detected` (bool), `started_in_cv` (bool); summary card charge rate and charge rate chart bars use `cc_rate_pct_per_hour` (skipping None entries) so CV taper does not drag down reported rates; charging sessions table adds "CC %/hr" and "Knee SOC" columns (shows "—" for CV-start and CC-only sessions)
  - **Diagnostics**: `_detect_thermal_derating(session_readings)` flags sessions where charger current drops >15% from peak. Peak = max current in the first 30 min (inrush window). Plateau = median of the *settled CC window* immediately after (readings window_idx → 1.5×window_idx) — NOT the second half of the session, which would include CV-phase taper and cause false positives on long complete charges. Step 6 suppression: if `_find_cc_cv_knee` finds a CC→CV knee AND `drop_idx ≤ window_idx`, the drop was inrush→CC settling, not thermal derating — suppress only when the drop is AT OR BEFORE the window boundary; `compute_diagnostics(charging_stats, discharge_stats, all_readings, charge_type_map, cfg_diag)` runs four checks: (1) thermal derating per session, (2) knee SOC drift vs rolling median baseline, (3) charge rate decline vs all-time average per type, (4) parasitic drain during idle periods (SOC drop with no session active); results persisted in `session_notes.flags` (per-session: `thermal_derating`, `knee_soc_low`, `charge_rate_declining`) and `system_diagnostics` table (parasitic drain); all thresholds configurable under `[diagnostics]` in config.ini; **`make_layout` in `app.py` calls `compute_and_save_diagnostics()` BEFORE `load_notes()` on initial page load** — this ordering is critical: notes carry diagnostic flags, so notes must be loaded after diagnostics are freshly written to avoid stale ⚠ icons in the charging table; panel shows at most 5 recent events per anomaly type with a collapsible "▶ N earlier events" `<details>` below; parasitic drain events consolidated into a single card regardless of count
  - SOC chart y-axis shows **Ah** when `battery_capacity_ah` is set (y = soc% × capacity/100), with % in hover (`X.X% / Y Ah`); falls back to SOC % when capacity is not configured. `build_figure` accepts `capacity_ah=None`; all callers in `app.py` and `report.py` pass `summary.get('capacity_ah')` or `data.get('capacity_ah')`.
  ...
  - Summary cards in four rows with hover tooltips:
    - **Usage**: 7-day avg %/day (+ Ah/day sublabel), all-days avg %/day (+ Ah/day), implied avg power (Option B: %/day × capacity × voltage ÷ 24); both averages are **hour-weighted** (total SOC drop ÷ total discharge hours × 24) so a brief 2-hour stop doesn't carry equal weight to a full boondocking day; summary stats always use all historical data regardless of the chart viewport time window (`--week`, `--days N` etc. only affect the chart's initial x-axis range)
